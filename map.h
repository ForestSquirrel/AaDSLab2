...
public:
	...
	List<haffman_pair<T, T1>>* get_pairs() {
		List<haffman_pair<T, T1>>* list = new List<haffman_pair<T, T1>>();
...
private:
	...
	void ListPairs(Node* tree, List<haffman_pair<T, T1>>* list) {
		if (tree != Leaf) {
			ListPairs(tree->left, list);
			haffman_pair<T, T1> current(tree->key, tree->value);
